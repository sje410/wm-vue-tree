<template>
  ...
  >
    ...
    >
      <template v-if="node.type === 0"><span class="tree-nodeTag" style="margin-left: 4px;"></span></template>
      <div
      ...
      <template v-if="node.type !== 2">
        ...
        >
          <node-content :node="node">
            <slot>
              <span :class="this.tree.store.status ? 'tree-node-name-status' : 'tree-node-name'"
      ...
      <template v-if="node.type === 2">
        <div
            class="org-chart-node-label-inner-text"
            @click="handleNodeClick"
            :class="computeLabelClass"
            :style="computeLabelStyle"
        >
